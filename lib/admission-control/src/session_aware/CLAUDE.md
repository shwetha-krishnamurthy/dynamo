...
| Dispatch the next turn of an assigned active session without applying new-program capacity admission again; periodic pressure handling pauses idle victims or marks running victims after completion. | This preserves upstream's active-program continuation semantics and prevents sustained backend underfill while retaining exact-worker cache locality. |
